  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (firebaseError.code === "auth/user-not-found" || firebaseError.code === "auth/invalid-email") {
        setResetError(t("auth.errorUserNotFound"));
      } else {
        setResetError(t("auth.errorGeneric"));
      }
    } finally {
        ...
        {forgotMode ? (
          ...
          <div>
            <div className="mb-8 text-center">
              <h1 className="text-3xl font-bold text-foreground">{t("auth.forgotPassword")}</h1>
            ...
            {resetSent ? (
              <div className="rounded-xl border border-green-200 bg-green-50 p-5 text-center dark:border-green-800 dark:bg-green-950/30">
                <svg className="mx-auto mb-3 h-8 w-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <p className="font-medium text-green-700 dark:text-green-400">{t("auth.resetEmailSent")}</p>
                <p className="mt-1 text-sm text-green-600 dark:text-green-500">{t("auth.resetEmailSentDesc")}</p>
                <button
              ...
              <form onSubmit={handleResetPassword} className="space-y-4">
                ...
                <div>
                  <label htmlFor="resetEmail" className="mb-1 block text-sm font-medium text-foreground">
                    {t("auth.email")}
                  </label>
                  <input
                    id="resetEmail"
                ...
                >
                  {resetLoading ? "..." : t("auth.sendResetEmail")}
                </button>
                <button
                  type="button"
